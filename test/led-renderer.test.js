...
test("display rendering never turns one Arrival into a weighted super-drop", () => {
  const pulse = {
    startedAt: 1_000,
    attack: 0.05,
    decay: 0.5,
  ...
  const output = sampleLedOutput([pulse], 1_050);

  assert.ok(Math.abs(output.levels[0] - 0.1) < 0.000001);
});
